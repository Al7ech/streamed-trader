...
import {AreaSeries, BaselineSeries, createSeriesMarkers, LineSeries, PriceScaleMode} from 'lightweight-charts';
import {Group, Paper, SegmentedControl, Text} from '@mantine/core';
import {CHART_COLORS, EQUITY_COLOR, TRADE_COLORS} from '../../theme';
 ...
 */
const EquityChart = ({equityData, mddRatio, timeSync}) => {
  ...
  useEffect(() => {
    ...
    const cleanupShell = installChartShell(chart, container, {
      ...
      // 실제 크기가 잡힌 뒤 한 번만 equity pane 이 85% 를 차지하도록 조정
      ...
      onFirstSize: () => {
        ...
        if (panes.length > 1 && totalHeight > 0) panes[0].setHeight(totalHeight * 0.85);
      },
    });
  ...
  }, [equityData, benchmarkData, drawdownData, trough]);
  ...
  return (
    ...
    >
      ...
      />
      {/* 시리즈가 2개 이상(equity + 기준선 + drawdown)이라 범례 필수 —
