...
- ``candles_by_symbol``을 넘기면 :class:`~core.engine.in_memory_candle_producer.InMemoryCandleProducer`
  가 그것을 병합한다 (합성 캔들 검사, 비-바이낸스 소스).
- ``producer``를 직접 넘기면 그대로 쓴다 — 보통
  :class:`~core.engine.binance_backtest_candle_producer.BinanceBacktestCandleProducer`
  로, 구간을 스스로 fetch한다.
"""
