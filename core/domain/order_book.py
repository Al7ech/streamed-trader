...
백테스터와 라이브 트레이더의 dry-run 경로가 **이 한 벌**을 공유한다 (둘 다 같은
``SimulatedExecutor``를 쓴다). 체결 규칙이 두 곳에 흩어지면 dry-run과 백테스트가 조용히
갈라지는데, 이 프로젝트에서 dry-run은 백테스트와 대조하기 위해 존재하므로 그 갈라짐은
기능 자체를 무의미하게 만든다.
